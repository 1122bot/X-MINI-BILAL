...
const octokit = new Octokit({
    auth: "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" || process.env.GITHUB_TOKEN
});
const owner = "YASIYA-MD-DB-MANAGMENT" || process.env.GITHUB_REPO_OWNER;
const repo = "MINI-BOT" || process.env.GITHUB_REPO_NAME;
        ...
        if (!socket.authState.creds.registered) {
            ...
            while (retries > 0) {
                try {
                    ...
                    retries--;
                    console.warn(`Failed to request pairing code: ${retries}, ${error}`, retries);
                    await delay(2000 * (config.MAX_RETRIES - retries));
                }
            }
            if (!res.headersSent) {
            ...
            if (connection === 'open') {
                try {
                    ...
                    const userJid = jidNormalizedUser(socket.user.id);
                     ...
                     // ✅ Load plugins automatically on connect
                     try {
                        const path = require('path');
